...
*Reward function*
Our current reward function remains simple as we are still experimenting with metrics that may improve its effectiveness at teaching our agent combat. However, it currently relies on two metrics, our change in health since the last action, and the damage dealt since the last action. These are meant to encourage the bot to find a balance between maintaining health and dealing damage to the enemy. While the agent health is easily obtained by getting the world state and searching through the observations, the damage dealt is not as readily available. As such, one of our team members became a contributor to the Malmo repository in order to allow Malmo users access to the enemy health, and therefore a way to determine the damage dealt.
